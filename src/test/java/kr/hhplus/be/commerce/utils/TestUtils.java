...
public abstract class TestUtils {
    ...
    public static User createTestUser() {
        return User.create("잘생긴라이언코치님");
    }

    public static Balance createTestBalance(Long userId) {
        return Balance.create(userId, 0L);
    }
    ...
    public static Product createTestProduct(long index, ProductStatus status, long stock) {
        return Product.create("멋쟁이라이언코치님께 질문고봉밥 스택 " + index,
                index * 1000,
                status,
                "라이언코치님께 " + index + "번 절하고 성장하기",
                stock);
    }
